	...
	<form action="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" method="post" class="std form-horizontal" id="add_address">
		...
		{foreach from=$ordered_adr_fields item=field_name}
			...
			{if $field_name eq 'address2'}
				<div class="required form-group">
					<label class="control-label col-sm-4" for="address2">{l s='Address (Line 2)'}{l s='Address (Line 2)'}{if isset($required_fields) && in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
					<div class="col-sm-6">
...
<ul class="footer_links list-unstyled clearfix">
	<li class="pull-left">
		<a class="btn btn-default button button-medium" href="{$link->getPageLink('addresses', true)|escape:'html':'UTF-8'}">
			<span><i class="icon-chevron-left left"></i> {l s='Back to your addresses'}</span>
		</a>
	</li>
</ul>
{strip}
