...
<div class="adresses_bloc">
	<div class="row">
		<div class="col-xs-12 col-sm-6"{if $order->isVirtual()} style="display:none;"{/if}>
			<ul class="list-unstyled address alternate_item box">
		...
		<div class="col-xs-12 col-sm-6">
			<ul class="list-unstyled address item {if $order->isVirtual()}full_width{/if} box">
					...
					{foreach $product.customizedDatas  as $customizationPerAddress}
						...
						<tr class="alternate_item">
							...
							{foreach from=$customization.datas key='type' item='datas'}
								...
								<ul class="list-unstyled typedText">{counter start=0 print=false}
									{foreach from=$datas item='data'}
...
{if !$is_guest}
	...
	{/if}
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std form-horizontal" id="sendOrderMessage">
