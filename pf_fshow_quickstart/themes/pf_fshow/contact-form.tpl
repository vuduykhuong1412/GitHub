	...
	<ul class="footer_links list-unstyled clearfix">
		<li>
            <a class="btn-default button btn" href="{$base_dir}">
                <span>
                    <i class="icon-chevron-left left"></i>{l s='Home'}
                </span>
            </a>
        </li>
	</ul>
...
{else}
	...
	<form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="contact-form-box box" enctype="multipart/form-data">
		<fieldset>
                        ...
                        {if isset($is_logged) && $is_logged}
                            <div class="form-group selector1">
                                ...
                                {elseif $customerThread.id_product > 0}
                                    <input class="form-control" type="hidden" name="id_product" id="id_product" value="{$customerThread.id_product|intval}" readonly="readonly" />
                                {/if}
                            </div>
                        {/if}
                    {/if}
