...
<div id="htmlcontent_{$hook|escape:'htmlall':'UTF-8'}" class="block {if {$hook|escape:'htmlall':'UTF-8'}=='footer'}col-md-3{/if}">
	<ul class="list-unstyled htmlcontent-home clearfix">
		{foreach name=items from=$htmlitems item=hItem}
			<li class="htmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'}">
				{*{if $hItem.title && $hItem.title_use == 1}*}
					<h4 class="item-title">{$hItem.title|escape:'htmlall':'UTF-8'}</h4>
				{*{/if}*}
				{if $hItem.url}
					<a href="{$hItem.url|escape:'htmlall':'UTF-8'}" class="item-link img-responsive"{if $hItem.target == 1} onclick="return !window.open(this.href);"{/if} title="{$hItem.title|escape:'htmlall':'UTF-8'}">
				{/if}
					{if $hItem.image}
