...
<div id="blog-listing" class="blogs-container">
 	{if isset($filter.type)}
	 	{if $filter.type=='tag'}
			<h1 class="page-heading">{l s='Filter Blogs By Tag' mod='leoblog'} : <span>{$filter.tag}</span></h1>
		{elseif $filter.type=='author'}
			<h1 class="page-heading">{l s='Filter Blogs By Blogger' mod='leoblog'} : <span>{$filter.employee->firstname} {$filter.employee->lastname}</span></h1>
		{/if}
	{else}
	<h1 class="page-heading">{l s='Lastest Blogs' mod='leoblog'}</h1>
	{/if}
			...
			<div class="leading-blog">  
				{foreach from=$leading_blogs item=blog name=leading_blog}
					...
					<div class="{if $listing_leading_column<=1}no{/if}col-lg-{floor(12/$listing_leading_column)}">
						 {include file="./_listing_blog_list.tpl"}
					</div>	
