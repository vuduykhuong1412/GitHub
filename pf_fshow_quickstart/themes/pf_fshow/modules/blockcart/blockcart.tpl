...
<div id="pts-mini-cart" class=" clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}
		...
		<div class="heading">
			<div class="mini-cart cart-inner">
				<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
					<span class="cart-title">{l s='My cart' mod='blockcart'}</span>
					<span class="ajax_cart_quantity mini-cart-count">{$cart_qties}</span>
					<!-- <span class="ajax_cart_product_txt{if $cart_qties != 1} unvisible{/if}">{l s='item' mod='blockcart'}</span> -->
					<span class="ajax_cart_product_txt_s hidden{if $cart_qties < 2} unvisible{/if}">{l s='items' mod='blockcart'}</span>
					<!--{l s='( ' mod='blockcart'} -->
						<span class="ajax_cart_total hidden {if $cart_qties == 0} unvisible{/if}">
							{if $cart_qties > 0}
								{if $priceDisplay == 1}
						...
						<span class="ajax_cart_no_product hidden{if $cart_qties > 0} unvisible{/if}">
							<span class="mini-cart-count">{l s='0' mod='blockcart'} <!--{l s='item' mod='blockcart'}--></span><span class="hidden">{convertPrice price=0}</span>
						</span>
					<!--{l s=' )' mod='blockcart'} -->
				...
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
								...
								<dl class="products">
									{foreach from=$products item='product' name='myLoop'}
										...
										{if isset($customizedDatas.$productId.$productAttributeId[$product.id_address_delivery])}
											...
											{/if}
											<ul class="list-unstyled cart_block_customizations" data-id="customization_{$productId}_{$productAttributeId}">
						...
						{if $discounts|@count > 0}
							<table class="vouchers mini-cart-vouchers{if $discounts|@count == 0} unvisible{/if}">
						...
						<div class="content-price">
							...
							<p class="cart-buttons clearfix">
								<a id="button_order_cart" class="btn button button-small pull-right" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
									<span>
					...
					<div class="row bg-white">
						...
						<div class="layer_cart_cart col-xs-12 col-md-6 bg-white">
							...
							<div class="button-container">	
								...
								<a class="btn btn-default button button-medium pull-right"	href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow">
									<span>
