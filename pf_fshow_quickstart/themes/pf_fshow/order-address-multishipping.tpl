...
<div class="addresses clearfix">
	...
	<div class="row">
    	<div class="col-sm-12 col-md-6">
            <ul class="list-unstyled address alternate_item {if $cart->isVirtualCart()}full_width{/if} box" id="address_invoice">
            </ul>
        </div>
	</div>
...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				{if $back}
					<a href="{$link->getPageLink('order', true, NULL, "back={$back}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default"><i class="icon-chevron-left left"></i>{l s='Continue Shopping'}</a>
				{else}
					<a href="{$link->getPageLink('order', true, NULL)|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default"><i class="icon-chevron-left left"></i>{l s='Continue Shopping'}</a>
				{/if}
		        <button type="submit" name="processAddress" class="button btn btn-default  button-medium pull-right"><span>{l s='Proceed to checkout'}<i class="icon-chevron-right right"></i></span></button>
			</p>
		</form>
	</div>
{else}
