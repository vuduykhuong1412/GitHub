...
{if isset($products) && $products}
	...
	{assign var='nbItemsPerLinesMobile' value=2}
	...
	{math equation="nbLi/nbItemsPerLine" nbLi=12 nbItemsPerLine=$nbItemsPerLinesMobile assign=scolMobile}
	<!-- Products list -->
	<script type="text/javascript">
		var colLap = {$colLap};
		var colTablet = {$colTablet};
		var colMobile = {$colMobile};
		var scolMobile = {$scolMobile};
	</script>
	<ul{if isset($id) && $id} id="{$id}"{/if} class="list-unstyled product_list products-block grid row{if isset($class) && $class} {$class}{/if} {if isset($product_style) &&  !empty($product_style)} {$product_style}{/if}">
	{foreach from=$products item=product name=products}
		...
		<li class="ajax_block_product  col-xs-12 col-xs-{$scolMobile} col-sm-{$colMobile} col-md-{$colTablet} col-lg-{$colLap}{if $smarty.foreach.products.iteration%$nbItemsPerLine == 0} last-in-line{elseif $smarty.foreach.products.iteration%$nbItemsPerLine == 1} first-in-line{/if}{if $smarty.foreach.products.iteration > ($smarty.foreach.products.total - $totModulo)} last-line{/if}{if $smarty.foreach.products.iteration%$nbItemsPerLineTablet == 0} last-item-of-tablet-line{elseif $smarty.foreach.products.iteration%$nbItemsPerLineTablet == 1} first-item-of-tablet-line{/if}{if $smarty.foreach.products.iteration%$nbItemsPerLineMobile == 0} last-item-of-mobile-line{elseif $smarty.foreach.products.iteration%$nbItemsPerLineMobile == 1} first-item-of-mobile-line{/if}{if $smarty.foreach.products.iteration > ($smarty.foreach.products.total - $totModuloMobile)} last-mobile-line{/if}" data-col-lg="{$colLap}" data-col-md="{$colTablet}" data-col-sm="{$colMobile}" data-col-xs="{$scolMobile}">
			{if isset($product_style) &&  !empty($product_style)} 
				{include file="$tpl_dir./sub/product/{$product_style}.tpl" product=$product class=''}
			{/if}
		</li>
		{if $smarty.foreach.products.iteration%$nbItemsPerLine==0 && !$smarty.foreach.products.last}
