...
<p class="clearfix">
  <a href="{$tools_editor|escape:'html':'UTF-8'}"   class="btn-danger btn pull-right" target="_blank" style="margin-left:10px;"><i class="icon-external-link-sign"></i> {l s='Live Theme Configurator' mod='ptsthemepanel'}</a> 
</p>
                ...
                <div class="panel-heading">
                  ...
                  <div class="panel panel-content">
                        ...
                        <div class="clearfix" id="btn-tools">
                          ...
                          <div class="pull-left">
                            <button type="submit" name="importConfiguration" class="btn btn-info btn-md btn-action">{l s='Import Sample Configuration' mod='ptsthemepanel'}</button>
                          

                            <button type="submit" name="importQueries" class="btn btn-md btn-warning btn-action">{l s='Import Sample Queries' mod='ptsthemepanel'}</button>
                            <button type="submit" name="restoreAll" class="btn btn-md btn-danger btn-action">{l s='Restore' mod='ptsthemepanel'}</button>

                            |   <button type="submit" name="assignHooksModulesInXml" class="btn btn-primary btn-md btn-action">{l s='Re-Assign Modules - Hooks' mod='ptsthemepanel'}</button> 
                          </div>

                          <div class="pull-right">
                             <button onclick="$('.pts-checkbox').attr('checked','checked')" type="submit" name="backupAll" class="btn btn-danger btn-md">
                              {l s='Backup' mod='ptsthemepanel'} <em>{l s='All Module Using To Restore Old' mod='ptsthemepanel'}</em>
                             </button>
                          </div>
                        ...
                        <div class="table-responsive">
                          <table class="table">
                            ...
                            <tr>
                              ...
                              <td>
                                <a href="{$moduleEditURL|escape:'html':'UTF-8'}&module_name={$sample.name|escape:'html':'UTF-8'}&configure={$sample.name|escape:'html':'UTF-8'}" class="btn btn-sm   btn-default liveeditmod">{l s='Live Configure' mod='ptsthemepanel'}</a>
                              </td>
                            </tr>
                            {/if}
                            {/foreach}
   ...
   <div class="panel">
        ...
        <div class="panel panel-content">
            <div class="alert alert-info">
                {l s='If you would like to hook or unhook modules to make look and feeld of theme as the demo. Please click to below button' mod='ptsthemepanel'} <br>
                <p> <button type="submit" name="assignHooksAllModulesInXml" class="btn btn-danger btn-md btn-action">{l s='Re-Assign All Modules - Hooks' mod='ptsthemepanel'}</button> </p>
            </div>
        </div>  
    </div>  
...
$(".liveeditmod, .pts-fancybox").fancybox({
  ...
  afterLoad:function(   ){
    if( $('body',$('.fancybox-iframe').contents()).find("#main").length  ){  
      ...
      $('body',$('.fancybox-iframe').contents()).find("#psException").html('<div class="alert error">{l s='No Configuration Avairiable' mod='ptsthemepanel'}</div>');
    }
  }
});
</script>
 ...
 //   $('#myTab a:first').tab('show')
  ...
  $("#btn-tools  button.btn-action").click( function() {  
      if( $('.pts-checkbox:checked').length > 0 ){
        ...
        alert( '{l s='Please select module(s) to do this action.' mod='ptsthemepanel'}');
        return false;
      }
      return true;
  } );
