	...
	$.fn.WPO_Layout = function( opts, datajson ) {
		...
		function updateColsWidth( col, dcol ){  
	  	...
	  	function addRow( srow, sub ){
	  		...
	  		var row = $( '<div class="wpo-row lg-row"><div class="wpo-tool"><div class="wpo-sortable" title="Sort"></div></div><div class="inner clearfix"></div></div>' );
	  		var edit = $('<div class="wpo-edit" title="Edit"></div>');
	  		var copy = $('<div class="wpo-copy" title="Duplicate"></div>');
	  		var del = $('<div class="wpo-delete" title="Delete"></div>');
	  	...
	  	function bindingRowEvents( row , srow ){
	  		...
	  		$( ".wpo-edit", row ).click( function(event){ 
	  			...
	  			var pos = $( row ).offset(); 
	  			var l = pos.left;
	  			var ll = $(this).offset().left - $("#nav-sidebar").width();
	  			var aa = $("#header_infos").height() + $(".page-head").height()  +36; 
	  	 

  				$( "#row-builder" ).css( {top:pos.top-$( "#row-builder" ).height()/2 - aa, left:ll} ).show();
